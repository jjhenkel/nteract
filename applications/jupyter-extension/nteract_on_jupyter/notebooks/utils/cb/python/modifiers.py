...
class CBAnyChildIs:
    ...
    def rewrite(node):
        ...
        copy1 = deepcopy(node).remove_mod(CBAnyChildIs)
        return new_root.nest(copy1)
...
class CBFirstChildIs:
    ...
    def rewrite(node):
        new_root = CB('$virtual').with_constraints(
            CBStepsAway(-1), CBFieldIndex('module_name')
        )
        copy1 = deepcopy(node).remove_mod(CBFirstChildIs)
        return new_root.nest(copy1)
